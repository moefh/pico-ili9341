...
static unsigned int conv_pixel_6bit(unsigned int sync_bits, unsigned int pixel)
{
  unsigned int r = (pixel>>16) & 0xff;
  unsigned int g = (pixel>> 8) & 0xff;
  unsigned int b = (pixel>> 0) & 0xff;
  if ((r>>6) == 0 && (g>>6) == 3 && (b>>6) == 0 && (g != 0xff)) {
    // don't convert almost-pure-green to pure green (transparent)
    r = 0;
    g = 2;
    b = 0;
  } else {
...
static unsigned int conv_pixel_8bit(unsigned int pixel)
{
  unsigned int r = (pixel>>16) & 0xff;
  unsigned int g = (pixel>> 8) & 0xff;
  unsigned int b = (pixel>> 0) & 0xff;
  ...
  for (int spr_num = 0; spr_num < info->num_frames; spr_num++) {
    ...
    for (int y = 0; y < spr->h; y++) {
      for (int lx = 0; lx < width/4; lx++) {  // process 4 pixels at a time
        ...
        unsigned int v;
        switch (info->conv_mode) {
        case CONV_MODE_6BIT_ESP32:
          v = ((conv_pixel_6bit(info->sync_bits, pixels[0])<<16) |
               (conv_pixel_6bit(info->sync_bits, pixels[1])<<24) |
               (conv_pixel_6bit(info->sync_bits, pixels[2])<< 0) |
               (conv_pixel_6bit(info->sync_bits, pixels[3])<< 8));
          break;
        case CONV_MODE_6BIT:
          v = ((conv_pixel_6bit(info->sync_bits, pixels[0])<< 0) |
               (conv_pixel_6bit(info->sync_bits, pixels[1])<< 8) |
               (conv_pixel_6bit(info->sync_bits, pixels[2])<<16) |
               (conv_pixel_6bit(info->sync_bits, pixels[3])<<24));
          break;
        case CONV_MODE_8BIT:
        default:
          v = ((conv_pixel_8bit(pixels[0])<< 0) |
               (conv_pixel_8bit(pixels[1])<< 8) |
               (conv_pixel_8bit(pixels[2])<<16) |
               (conv_pixel_8bit(pixels[3])<<24));
          break;
        }
        if (num_out++ % 8 == 0) {
...
static void print_help(const char *progname)
{
  ...
  printf("   -sync BITS       set sync bits (default: 0xc0)\n");
  printf("   -num-frames N    set maximum num frames to convert (defaut: 0=unlimited)\n");
  printf("   -mode MODE       select mode: one of '6bit', '6bit-esp32', '8bit'\n");
  ...
  for (int i = 1; i < argc; i++) {
    if (argv[i][0] == '-') {
      if (strcmp(argv[i], "-h") == 0) {
        ...
        i++;
        if (strcmp(argv[i], "6bit") == 0) {
          info->conv_mode = CONV_MODE_6BIT;
        } else if (strcmp(argv[i], "6bit-esp32") == 0) {
          info->conv_mode = CONV_MODE_6BIT_ESP32;
        } else if (strcmp(argv[i], "8bit") == 0) {
          info->conv_mode = CONV_MODE_8BIT;
        } else {
          printf("%s: invalid mode: '%s'\n", info->progname, argv[i+1]);
          return 1;
        }
